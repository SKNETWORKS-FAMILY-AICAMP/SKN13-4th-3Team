...
####################################################
class CarImageTextDataset(Dataset):
...
def main():
    ...
    # 직접 CarImageTextDataset에 tokenizer를 넘겨서 데이터셋을 커스터마이즈할 거기 때문에
    # 기존 tokenizer 정의는 필요 없음.
    # tokenizer = CLIPTokenizer.from_pretrained(
    #     args.pretrained_model_name_or_path, subfolder="tokenizer", revision=args.revision
    # )
    # ✅ tokenizer 및 이미지 전처리기 로드
    tokenizer = CLIPTokenizer.from_pretrained("sd-legacy/stable-diffusion-v1-5", subfolder="tokenizer")
    image_processor = StableDiffusionPipeline.from_pretrained("sd-legacy/stable-diffusion-v1-5").feature_extractor
    ...
    # 원래 data 불러오는 과정은 huggingface hub or dir에서 이미지를 불러올 수 있게 설정되어있음.
    # 그래서 맨위에 클래스로 Json 파일을 불러와서 Dataset을 읽게 수정.
