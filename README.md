# SKN13-4th-3Team

### 4차 프로젝트: JJACKLETTE - 기업 특화된 이미지 생성 AI 챗봇<br>

**개발기간:** 2025.07.18 ~ 2025.07.22

## 🐥 팀 소개
## 팀명: 밥심 🍚

<!-- | **기원준🥚** | **전진혁🐣** | **강지윤🐥** | **최호연🐓** | | **우민규🍗** |
|:--------------:|:--------------:|:--------------:|:--------------:|
| @ki-student     | @Jinhyeok33 | @jiyun-kang12 | @oowixj819 | | @mingyu-oo | -->


<table>
  <tr>
    <td align="center">
      <img src="https://github.com/user-attachments/assets/50dc49bf-6b22-46f4-b98b-ca471b23e5e7" width="100"/><br/>기원준🥚
    </td>
    <td align="center">
      <img src="https://github.com/user-attachments/assets/b3003734-50d5-4a7c-93a0-468345b23d1a" width="100"/><br/>전진혁🐣
    </td>
    <td align="center">
      <img src="https://github.com/user-attachments/assets/85c59737-8172-4319-ac86-f1a766d83dab" width="100"/><br/>강지윤🐥
    </td>
    <td align="center">
      <img src="https://github.com/user-attachments/assets/7d5d501e-a319-4704-8f15-e593fcf2f90f" width="100"/><br/>최호연🐓
    </td>
    <td align="center">
      <img src="https://github.com/user-attachments/assets/af8e3f73-c42f-451d-b3c9-1234aa6328cf" width="100"/><br/>우민규🍗
    </td>
  </tr>
  <tr>
    <td align="center">
      <a href="https://github.com/ki-student"><img src="https://img.shields.io/badge/GitHub-ki--student-1F1F1F?logo=github" alt="기원준 GitHub"/></a>
    </td>
    <td align="center">
      <a href="https://github.com/Jinhyeok33"><img src="https://img.shields.io/badge/GitHub-Jinhyeok33-1F1F1F?logo=github" alt="전진혁 GitHub"/></a>
    </td>
    <td align="center">
      <a href="https://github.com/jiyun-kang12"><img src="https://img.shields.io/badge/GitHub-jiyun--kang12-1F1F1F?logo=github" alt="강지윤 GitHub"/></a>
    </td>    <td align="center">
      <a href="https://github.com/oowixj819"><img src="https://img.shields.io/badge/GitHub-oowixj819-1F1F1F?logo=github" alt="최호연 GitHub"/></a>
    </td>    <td align="center">
      <a href="https://github.com/mingyu-oo"><img src="https://img.shields.io/badge/GitHub-mingyu--oo-1F1F1F?logo=github" alt="우민규 GitHub"/></a>
    </td>
</table>


### 🐥 1. 프로젝트 개요

#### 1.1 개발 동기 및 목적

- 오늘날의 제품 디자인은 단순히 기능적 완성도를 넘어서, 고객의 일상과 감정을 이해하고 그것을 제품에 반영하는<br>
  **사용자 경험(User Experience, UX) 중심의 접근**이 필수적임.<br>
- 하지만 고객의 니즈는 항상 명확하게 표현되지 않으며, 디자이너는 고객이 직접 말로 표현하지 못하는 니즈까지 파악하고<br>
  이를 창의적으로 구현할 수 있어야 함.<br>
- 이러한 문제를 해결하기 위해 본 프로젝트는 단계별 문제 해결 프레임워크 중 하나인<br>
   **디자인 씽킹(Design Thinking)**,  특히 McKinsey의 **다이아몬드 모델**에 주목하였음.

McKinsey 다이아몬드 모델은 다음과 같은 단계로 UX와 SX를 치밀하게 설계하는 접근법:<br>
     - **Research → Define → Ideate → Prototype → Launch and scale**

<p align="center">
  <img src="https://github.com/user-attachments/assets/b034b1db-eec1-4ca5-8670-25f8ad43f80e" width="600" />
</p>
<p align="center">
  <sub><i>이미지 출처: https://www.mckinsey.com/industries/metals-and-mining/our-insights/the-diamond-industry-is-at-an-inflection-point</i></sub>
</p>

‼️ 그러나, 현실에서는 이 모든 단계를 디자이너가 직접 하나하나 수행할 경우, **시간적·비용적 부담**이 크게 발생하며, 여러 아이디어를 시도해볼 기회도 제한됨<br> 
    - 이는 프로토타입을 빠르게 반복 제작하고 실험할 여유가 부족해지면서, 혁신적인 아이디어의 실현 가능성도 줄어들게 됨. <br>
    
📌 이러한 문제에 대응하기 위한 전략적 프로세스로 아래 그림과 같은 린"Lean" 은 동일한 목표를 공유하고, 빠르고, 불필요한 요소를 줄이고 <br>
    사용자에게 효과적인 경험을 디자인 할 수 있는 프로세스로 주목 받고 있으며,<br>
    린"Lean" 은 크게 3가지 단계가 있고, 이 단계들은 최종적인 디자인 완성까지 지속적으로 순환되는 형식으로 구성되어 있음. 


<p align="center">
  <img src="https://github.com/user-attachments/assets/2accb349-f1bd-42c4-8c95-e50f0dfd93d3" width="500" />
</p> <p align="center">
  <sub><i>이미지 출처: https://www.jungle.co.kr/magazine/6728</i></sub>
</p>
  
---

#### 1.2.🛠️ 개발 목표  

본 프로젝트는 이러한 문제를 해결하고자:

- ✅ 제품 디자이너들이 **고객의 목소리(VoC)를 신속히 분석·이해**하고,<br>
고객의 목소리(VoC)를 AI가 분석해 제품 설계 및 개선에 반영함으로써, 사용자 중심 설계(User-Centered Design)를 강화<br>

- ✅ **자사 고유의 디자인 아이덴티티를 유지**하며,<br>
  회사 고유의 디자인 아이덴티티와 철학을 sLLM에 학습시켜, 모든 제품 및 서비스에서 브랜드 정체성이 흔들리지 않도록 함.
  
- ✅ **개인화되고 차별화된 프로토타입**을 손쉽게 생성할 수 있도록 돕는 **sLLM 기반 AI 챗봇**을 개발하고자 함.

#### 1.3. 본 챗봇의 차별성

1️⃣ 우리만의 UX 지식 반영<br>
일반 LLM이 아닌, 회사 고유의 디자인 전략과 철학, 과거 성공 사례, UX 지식이 학습된 sLLM을 사용.<br>
이를 통해 ‘우리 회사답다’라는 느낌이 제품과 서비스에 자연스럽게 녹아들며, 브랜드 일관성과 차별성을 동시에 확보할 수 있음.<br>

2️⃣ 안전한 데이터 처리 (보안 강화)<br>
사용자 경험 데이터에는 개인 정보 및 민감한 선호도가 포함되어 있기 때문에, sLLM은 고도화된 보안 알고리즘을 통해 데이터 보호를 최우선으로 함.<br>
모델 학습 및 서비스 제공 단계에서 데이터 유출, 무단 접근, 오남용을 방지하여 신뢰성을 확보.<br>

3️⃣ 협업 효율화 및 개인화된 AI 지원<br>
디자이너, 엔지니어, 기획자 간 협업 과정에서 챗봇이 디자인 아이디어 정제, 고객 피드백 분석, 레퍼런스 추천 등의 역할을 수행함.<br>
또한, 개인별로 다른 작업 스타일과 선호에 맞춘 개인화된 AI 보조로, 생산성을 높이고 창의적 작업에 더 많은 시간을 쓸 수 있도록 함.<br>


#### 🔧주요 기능

**🛠 UX 리서치 지원**
- 인터뷰·설문·사용성 테스트 데이터 요약
- 리뷰 데이터 분석 및 개선 인사이트 추천

**🎨 디자인 지원**
- 이미지·스케치 기반 프로토타입 추천
  

**🧠 UX 전략·분석**
- 사용자 여정맵(Journey Map) 자동 작성
- A/B 테스트 결과 요약 및 액션 제안

#### 🔧 기술적 특징

본 챗봇은 디자이너들의 창의적 작업을 지원할 뿐만 아니라,  
사용자 경험 데이터를 분석하고, 시각화된 프로토타입을 생성하는 AI 모델로서 다음을 특징으로 합니다:

✅ **image-to-text / text-to-image 모델을 활용한 fine-tuning** 
- 시각 정보 이해 및 생성에 특화된 멀티모달 AI 구조
- 고객 리뷰, VOC 등 텍스트 데이터로부터 인사이트를 추출 

✅ **vision encoder 모델 설계**  
- 이미지 및 스케치 데이터를 해석해 컨셉 제안 및 프로토타입 생성

✅ **LangGraph를 활용한 멀티모달 챗봇 설계**  
- 텍스트 답변과 이미지 생성/추천을 분리해 처리하며,  
상황에 맞는 최적의 응답(텍스트/이미지)을 제공하는 워크플로우 설계


### 🐥 2. 기술 스택
| **분야**              | **기술 및 라이브러리**                                                                                                                                                                                                                            |
| ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 🖥️ 프로그래밍 언어 & 개발환경 | <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=Python&logoColor=white" /> <img src="https://img.shields.io/badge/VS%20Code-007ACC?style=for-the-badge&logo=VisualStudioCode&logoColor=white" />            |
| 🔗 LLM 체인 및 워크플로우   | <img src="https://img.shields.io/badge/LangChain-005F73?style=for-the-badge&logo=LangChain&logoColor=white" /> <img src="https://img.shields.io/badge/LangGraph-000000?style=for-the-badge&logo=LangChain&logoColor=white" />             |
| 🧠 LLM 모델           | <img src="https://img.shields.io/badge/OpenAI%20GPT--4.1-412991?style=for-the-badge&logo=OpenAI&logoColor=white" /> <img src="https://img.shields.io/badge/OpenAI%20Embeddings-10A37F?style=for-the-badge&logo=OpenAI&logoColor=white" /> |
| 🖼️ 사용한 모델        | <img src="https://img.shields.io/badge/InternVL3--8B-FF6F61?style=for-the-badge&logo=OpenAI&logoColor=white" /> <img src="https://img.shields.io/badge/stable--diffusion--v1--5-4F8A8B?style=for-the-badge&logo=StableDiffusion&logoColor=white" /> |
| 📄 문서 로딩 및 전처리      | <img src="https://img.shields.io/badge/PyMuPDF-00599C?style=for-the-badge&logo=AdobeAcrobatReader&logoColor=white" /> <img src="https://img.shields.io/badge/pandas-150458?style=for-the-badge&logo=pandas&logoColor=white" />            |
| 📦 벡터 저장소 및 임베딩     | <img src="https://img.shields.io/badge/qdrant-E34F26?style=for-the-badge&logo=qdrant&logoColor=white">                                                                                                                                |
| 🌐 데이터 수집           | <img src="https://img.shields.io/badge/Selenium-43B02A?style=for-the-badge&logo=Selenium&logoColor=white" /> <img src="https://img.shields.io/badge/requests-7A88CF?style=for-the-badge&logo=Python&logoColor=white" />                   |
| 🔐 환경 변수 및 설정 관리    | <img src="https://img.shields.io/badge/python_dotenv-000000?style=for-the-badge&logo=Python&logoColor=white" />                                                                                                                           |
| 💬 메시지 및 커뮤니케이션     | <img src="https://img.shields.io/badge/Discord-5865F2?style=for-the-badge&logo=Discord&logoColor=white" />                                                                                                                                |
| 📁 협업 및 형상 관리       | <img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=Git&logoColor=white" /> <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=GitHub&logoColor=white" />  


## 🐥 3. 요구사항 명세서


### 기능 요구사항 (Functional Requirements)

| 요구사항 ID | 기능유형 | 요구사항명      | 요구사항 내용                                               | 중요도 | 담당자 | 수용여부 |
| :---------: | :------: | :------------- | :--------------------------------------------------------- | :----: | :----: | :------: |
| BAB-001     | 기능     | 회원관리       | 회원 가입, 로그인, 정보 수정, 탈퇴 등의 기능을 제공해야 함. | 상     |        | 수용     |
| BAB-002     | 기능     | 챗봇           | 일반 질문, 이미지 검색의 기능을 제공해야 함.               | 상     |        | 수용     |
| BAB-003     | 비기능   | 반응형 디자인  | 디바이스 크기에 맞춰 디스플레이를 자동조정해야 함.         | 상     |        | 수용     |
| BAB-004     | 비기능   | 보안           | 안전해야 함.                                               | 상     |        | 수용     |
| BAB-005     | 비기능   | 성능           | 응답 시간이 2초 이내여야 함.                               | 하     |        | 수용     |
| BAB-006     | 기능     | 프로필 수정    | 닉네임, 이메일 수정이 가능해야 함.                         | 중     |        | 수용     |


### 🐥 4. 파일구조

📦SKN13-4th-3Team<br>
┣ 📂Qdrant_DB<br>
┃ ┣ 📂collection<br>
┃ ┃ ┗ 📂InternVL3_vector_store<br>
┃ ┃ ┃ ┗ 📜storage.sqlite<br>
┃ ┗ 📜meta.json<br>
┣ 📂Tools<br>
┃ ┗ 📜tools.py<br>
┣ 📂accounts<br>
┃ ┣ 📂__pycache__<br>
┃ ┃ ┣ ...<br>
┃ ┣ 📂migrations<br>
┃ ┃ ┣...
┃ ┣ 📂templates<br>
┃ ┃ ┗ 📂accounts<br>
┃ ┃ ┃ ┣ ...<br>
┃ ┣ ...
┣ 📂chat_bot<br>
┃ ┣ 📂__pycache__<br>
┃ ┃ ┣ ...<br>
┃ ┣ 📂migrations<br>
┃ ┃ ┣ 📂__pycache__<br>
┃ ┃ ┃ ┗ 📜__init__.cpython-312.pyc<br>
┃ ┃ ┣ ...<br>
┃ ┣ 📂rag<br>
┃ ┃ ┣ 📜prompt_template.py<br>
┃ ┃ ┗ 📜retriever.py<br>
┃ ┣ 📂templates<br>
┃ ┃ ┗ 📂chat_bot<br>
┃ ┃ ┃ ┣ 📜chat_bot.html<br>
┃ ┃ ┃ ┗ 📜chat_converation.html<br>
┃ ┣ ...
┣ 📂config<br>
┃ ┣ 📂__pycache__<br>
┃ ┃ ┣ ...<br>
┃ ┣ ...
┣ 📂data<br>
┃ ┗ 📜InternVL3_8B_result_dataset.json<br>
┣ 📂images<br>
┃ ┗ 📜.DS_Store<br>
┣ 📂main<br>
┃ ┣ 📂__pycache__<br>
┃ ┃ ┣ ...<br>
┃ ┣ 📂migrations<br>
┃ ┃ ┣ 📂__pycache__<br>
┃ ┃ ┃ ┗ 📜__init__.cpython-312.pyc<br>
┃ ┃ ┗ 📜__init__.py<br>
┃ ┣ 📂templates<br>
┃ ┃ ┗ 📂main<br>
┃ ┃ ┃ ┣ 📜base.html<br>
┃ ┃ ┃ ┗ 📜home.html<br>
┃ ┣ ...
┣ 📂model_core<br>
┃ ┣ ...<br>
┣ 📂qdrant<br>
┃ ┣ 📜.lock<br>
┃ ┗ 📜meta.json<br>
┣ 📂templates<br>
┃ ┗ 📜base.html<br>
┣ 📂Code<br>
┃ ┗ 📜InternVL3<br>
┃ ┃ ┣ ...
┃ ┗ 📜model_core<br>
┃ ┃ ┣ ...
┃ ┗ 📜stable-diffusion<br>
┃ ┃ ┣ ...
┣ 📂산출물<br>
┃ ┗ 📜System Architecture.pdf<br>
┃ ┗ 📜Langgraph.pdf<br>
┃ ┗ 📜4차프로젝트__UI설계.png<br>
┃ ┗ 📜test.md<br>
┣ 📜.DS_Store<br>
┣ 📜.gitignore<br>
┣ 📜README.md<br>
┣ 📜db.sqlite3<br>
┣ 📜manage.py<br>
┗ 📜requirements.txt<br>




 ### 🐥 5. 데이터 수집 및 전처리


 🏷️ **메타데이터 태깅 기준**
- Qdrant 벡터DB에 넣을 문서 데이터를 구성할 때,  
자동차 설명(description) 데이터와 사용자 피드백(review) 데이터를 각각 전처리하고,  
메타데이터로 태깅하여 검색·추천에 활용할 수 있도록 설계

#### 🚗 1️⃣ Description Vector DB (`description_vector_store`)

**데이터 소스**  
- `hyundaicar_descript_merge_all.json`

**주요 전처리 및 태깅**
- 자동차 이름 처리  
  - `car_name_en`: 영문 이름 (`_` 제거, `strip()`)  
  - `car_name_kr`: 영문 이름에서 'Hybrid' → '하이브리드'로 변환

- 설명 텍스트에서 영어 키워드 추출  
  - 키워드 목록: `["sleek", "modern", "aerodynamic", "SUV", "sedan", "electric", "hybrid", "dynamic", "spacious", "luxury"]`  
  - `CountVectorizer`로 등장 키워드만 추출

- **최종 메타데이터 예시**
  ```json
  {
    "description_keywords": ["sleek", "SUV"],
    "url": "image_path"
  }

#### 📝 2️⃣ Feedback Vector DB (`feedback_vector_store`)

**데이터 소스**  
- `hyundai_car_reviews.json`

---

**주요 전처리 및 태깅**

- 🚗 **자동차 이름 처리**  
  - `car_name_kr`: 한글 이름 (`strip()`)  
  - `car_name_en`: 한글 이름에서 공백 제거, `'하이브리드' → 'Hybrid'`로 변환

- 🏷️ **리뷰 텍스트에서 한글 키워드 추출**  
  - 키워드 목록:  
    `["연비", "주행감", "실내공간", "디자인", "하이브리드", "정숙성", "가속", "승차감", "가격", "안전성"]`  
  - 포함 여부 단순 필터링

- 🔢 **리뷰 길이 계산**  
  - `len(review_text)`

- 🗂️ **tags 딕셔너리 평탄화**
  ```python
  # 예시
  { "option": "파노라마 선루프", "color": "화이트" }
  → { "tags_option": "파노라마 선루프", "tags_color": "화이트" }

#### 📦 공통 처리

✂️ **문서 쪼개기 (Text Splitting)**  
- `RecursiveCharacterTextSplitter` 사용  
  `chunk_size=300`, `chunk_overlap=30`

🧠 **벡터 임베딩**  
- `OpenAIEmbeddings(model="text-embedding-3-large")`

💾 **Qdrant 저장**  
- 벡터 차원 (`EMBEDDING_DIM`): `3072`  
- 거리 계산 방식: `cosine`


 ### 🐥 6. RAG 기반 챗봇 구현🤖


 🔧**시스템 아키텍처**<br>
<img src="https://github.com/user-attachments/assets/145a2613-35c6-4bc7-8acd-94120557785a" width="800" alt="system"> <br>


 🔧**UI 화면 설계서**<br>
<img src="https://github.com/user-attachments/assets/7b1b2875-0c88-46c7-8d45-4eb2fab2391d" width="800" alt="system"> <br>




### 🐥 7. 챗봇 구현 화면
🖥️ 메인 화면
<img width="1912" height="960" alt="image" src="https://github.com/user-attachments/assets/26aecfb7-7a4e-46ec-b126-e4454ae98579" />

🖥️ 챗봇 화면
<img width="1919" height="976" alt="image" src="https://github.com/user-attachments/assets/160a1ed3-ea51-498a-918e-80469da64c06" />



### 🐥 8. 한줄 회고

 - 🥚기원준: AWS EC2 "Legend Server"
 - 🐣전진혁: docker master
 - 🐥강지윤: "image to text" InternVL3 master 
 - 🐓최호연: django master
 - 🍗우민규: README를 잘 읽자
